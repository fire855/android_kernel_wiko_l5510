...
static int msm_otg_set_power(struct usb_phy *phy, unsigned mA)
{
	 ...
	 */
	pr_info("msm_otg_set_power  lishengli %d\n", motg->chg_type);    //add by lishengli 20140911
	if (motg->chg_type == USB_SDP_CHARGER)
...
static void msm_otg_sm_work(struct work_struct *w)
{
	...
	pr_debug("%s work\n", usb_otg_state_string(otg->phy->state));
	pr_info("%s work lishengli\n", usb_otg_state_string(otg->phy->state));    //add by lishengli 20140911
	...
	case OTG_STATE_B_IDLE:
		if (test_bit(MHL, &motg->inputs)) {
			...
			pr_debug("b_sess_vld\n");
			pr_info("%d b_sess_vld lishengli\n", motg->chg_state);    //add by lishengli 20140911
			switch (motg->chg_state) {
			case USB_CHG_STATE_UNDEFINED:
				msm_chg_detect_work(&motg->chg_work.work);
				break;
			case USB_CHG_STATE_DETECTED:
				pr_info("%d USB_CHG_STATE_DETECTED lishengli\n", motg->chg_type);    //add by lishengli 20140911
				...
				case USB_SDP_CHARGER:
					msm_otg_notify_charger(motg,				//add by lishengli 20141106 	can charging by off-standard charger
							IDEV_ACA_CHG_LIMIT);
					msm_otg_start_peripheral(otg, 1);
					otg->phy->state =
						OTG_STATE_B_PERIPHERAL;
	...
	case OTG_STATE_B_PERIPHERAL:
		if (test_bit(B_SESS_VLD, &motg->inputs) &&
			...
			pr_info("OTG_STATE_B_PERIPHERAL ID_C lishengli\n");    //add by lishengli 20140911
			msm_otg_notify_charger(motg, IDEV_ACA_CHG_MAX);
		}
		break;
	case OTG_STATE_B_WAIT_ACON:
		if (!test_bit(ID, &motg->inputs) ||
			...
			pr_info("OTG_STATE_B_WAIT_ACON ID_C lishengli\n");    //add by lishengli 20140911
			msm_otg_notify_charger(motg, IDEV_ACA_CHG_MAX);
		}
		break;
	case OTG_STATE_B_HOST:
		if (!test_bit(B_BUS_REQ, &motg->inputs) ||
			...
			pr_info("OTG_STATE_B_HOST ID_C lishengli\n");    //add by lishengli 20140911
			msm_otg_notify_charger(motg, IDEV_ACA_CHG_MAX);
		}
		break;
	case OTG_STATE_A_IDLE:
		...
		if (test_bit(ID, &motg->inputs) &&
			...
			clear_bit(A_BUS_DROP, &motg->inputs);
			if (test_bit(ID_A, &motg->inputs)) {
					pr_info("OTG_STATE_A_IDLE ID_A lishengli\n");    //add by lishengli 20140911
					msm_otg_notify_charger(motg,
							IDEV_ACA_CHG_MAX);
			} else if (!test_bit(ID, &motg->inputs)) {
				msm_otg_notify_charger(motg, 0);
	...
	case OTG_STATE_A_HOST:
		if ((test_bit(ID, &motg->inputs) &&
			...
			msm_otg_del_timer(motg);
			msm_hsusb_vbus_power(motg, 0);
			pr_info("OTG_STATE_A_HOST ID_A lishengli\n");    //add by lishengli 20140911
			if (motg->chg_type == USB_ACA_DOCK_CHARGER)
				msm_otg_notify_charger(motg,
						IDEV_ACA_CHG_MAX);
			else
