...
void mdss_dsi_ctrl_init(struct mdss_dsi_ctrl_pdata *ctrl)
{
	...
	//LINE <lcm> <DATE20141218> <read more register> limi.zhang
...
static int mdss_dsi_read_status_two(struct mdss_dsi_ctrl_pdata *ctrl)
{
	struct dcs_cmd_req cmdreq;

	memset(&cmdreq, 0, sizeof(cmdreq));
	cmdreq.cmds = ctrl->status_cmds_two.cmds;
	cmdreq.cmds_cnt = ctrl->status_cmds_two.cmd_cnt;  
	cmdreq.flags = CMD_REQ_COMMIT | CMD_CLK_CTRL | CMD_REQ_RX;
	cmdreq.rlen = 0;
	cmdreq.cb = NULL;
	cmdreq.rbuf = ctrl->status_buf_two.data;
...
int mdss_dsi_reg_status_check(struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
    ...
    //BEGIN <lcm> <DATE20141218> <read more register> limi.zhang
	temp = mdss_dsi_read_status(ctrl_pdata);
     
       if(0x02 == ctrl_pdata->status_read_mode)              
       temp1 = mdss_dsi_read_status_two(ctrl_pdata); 
    ...
    if(0x02 == ctrl_pdata->status_read_mode)
    {
        ...
        //pr_info("%s:limi.zhang11==> ctrl->status_buf_two.data[0]=%d, ctrl_pdata->status_read_mode=%d\n", __func__,ctrl_pdata->status_buf_two.data[0], ctrl_pdata->status_read_mode);
		if (temp1 == 0 && temp == 0) {
			if ((ctrl_pdata->status_buf_two.data[0] !=ctrl_pdata->status_value_two) || (ctrl_pdata->status_buf.data[0] !=ctrl_pdata->status_value))
							 {
			...
			pr_err("%s: Read status register two returned error\n", __func__); // no return
		}
    }
	else 
	{
