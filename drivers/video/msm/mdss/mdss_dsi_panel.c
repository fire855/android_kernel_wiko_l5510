...
extern void gpio_clk_led_set(int level);
#endif
...
{
	struct mdss_dsi_ctrl_pdata *ctrl_pdata = NULL;
	//<BEGIN><><20150129><modify brightness for l9010>xiongdajun
	#if defined(CONFIG_PROJECT_L9010)	
	struct mipi_panel_info *mipi;
	struct mdss_dsi_ctrl_pdata *ctrl = NULL;
	#endif
	//<END><><20150129><modify brightness for l9010>xiongdajun
	if (pdata == NULL) {
		pr_err("%s: Invalid input data\n", __func__);
		return;
	}
	...
	#if defined(CONFIG_LEDS_GPIO_CLK)	
		gpio_clk_led_set(bl_level); //LINE<20141204>wangyanhui delete for PR3
	#endif
		break;
	case BL_DCS_CMD:
...
//<BEGIN><><20150129><modify brightness for l9010>xiongdajun
	...
	case BL_CMD:
		...
		if (ctrl->bright_cmds.cmd_cnt) {
			ctrl->bright_cmds.buf[ctrl->bright_cmds.blen-1] = bl_level;
			mdss_dsi_panel_cmds_send(ctrl, &ctrl->bright_cmds);
			}
...
static int mdss_dsi_panel_on(struct mdss_panel_data *pdata)
{
	struct mipi_panel_info *mipi;
	struct mdss_dsi_ctrl_pdata *ctrl = NULL;
	//<BEGIN><><20150203><add ESD check for l9010>xiongdajun
	#if defined(CONFIG_PROJECT_L9010)
	struct dcs_cmd_req cmdreq;
	#endif
	//<END><><20150203><add ESD check for l9010>xiongdajun
	if (pdata == NULL) {
		pr_err("%s: Invalid input data\n", __func__);
		return -EINVAL;
	}
				       ...
				       struct mdss_panel_info *panel_info)
{
	...
	//BEGIN <LCM> <DATE20150203> <add TE check for lcm eds check> limi.zhang 
	 	 panel_info->te.te_gpio = of_get_named_gpio_flags(np, "qcom,irq-te",
	 				    0, &panel_info->te.te_flags);
	 	 if (panel_info->te.te_gpio < 0) {
	 		 pr_err("%s:%d get te gpio fail\n",__func__,__LINE__);
	 	  }else{
	 		  pr_info("liyong get te gpio success \n");
	 	  }
	//END <LCM> <DATE20150203> <add TE check for lcm eds check> limi.zhang 	  
}
...
#if defined(CONFIG_LEDS_GPIO_CLK)	
		else if (!strncmp(data, "bl_ctrl_gp_clk", 14)) {
			ctrl_pdata->bklt_ctrl = BL_GP_CLK;
		}
#endif
//<BEGIN><><20150129><modify brightness for l9010>xiongdajun
#if defined(CONFIG_PROJECT_L9010)	
		else if (!strncmp(data, "bl_ctrl_cmd", 11)) {
			ctrl_pdata->bklt_ctrl = BL_CMD;
		}
#endif
//<END><><20150129><modify brightness for l9010>xiongdajun
	}
	...
	"qcom,mdss-dsi-bright-command", "qcom,mdss-dsi-bright-command-state");
	#endif
	//<BEGIN><><20150129><modify brightness for l9010>xiongdajun
    ...
    //BEGIN <lcm> <DATE20141218> <read more register> limi.zhang
	mdss_dsi_parse_dcs_cmds(np, &ctrl_pdata->status_cmds_two,
			"qcom,mdss-dsi-panel-status-command_two",
				"qcom,mdss-dsi-panel-status-command-state");
	
	rc = of_property_read_u32(np, "qcom,mdss-dsi-panel-status-value_two", &tmp);
	ctrl_pdata->status_value_two = (!rc ? tmp : 0);

	ctrl_pdata->status_read_mode = 0;
    
	rc = of_property_read_string(np,
				"qcom,mdss-dsi-panel-status-read-mode", &data);
	if (!rc) {
		if (!strcmp(data, "two"))
			ctrl_pdata->status_read_mode = 0x02;
		else 
			ctrl_pdata->status_read_mode = 0x01;
	}
	else{
	    ctrl_pdata->status_read_mode = 0;
	}
...
//BEGIN <LCM> <DATE20150203> <add TE check for lcm eds check> limi.zhang 	
	 	rc = of_property_read_string(np,
	 				"qcom,mdss-dsi-TE-mode", &data);
	 	if (!rc) {
	 		if (!strcmp(data, "on"))
	 			ctrl_pdata->status_TE_mode = 0x01;
	 		else if(!strcmp(data, "off"))
	 			ctrl_pdata->status_TE_mode = 0;
	 	}
	 	else{
	 	    ctrl_pdata->status_TE_mode = 0;
	 	}
//END <LCM> <DATE20150203> <add TE check for lcm eds check> limi.zhang 
	...
	pr_info("limi.zhang==> qcom,mdss-dsi-panel-status-value_two tmp=0x%x",ctrl_pdata->status_value_two);
	//END <lcm> <DATE20141218> <read more register> limi.zhang
...
static irqreturn_t te_interrupt(int irq, void *data)
{
       ...
       if(te_count++%50==0)
               pr_info("%s liyong count = %d\n", __func__, te_count);
       enable_irq(irq);
       return IRQ_HANDLED;
}

int init_te_irq(struct mdss_panel_info *pinfo)
{
       int rc = -1;

       if (gpio_is_valid(pinfo->te.te_gpio)) {
               rc = gpio_request(pinfo->te.te_gpio, "te-gpio");
               if (rc < 0) {
                       pr_err("%s: gpio_request fail rc=%d\n", __func__,rc);
                       return rc ;
               }

               rc = gpio_direction_input(pinfo->te.te_gpio);
               if (rc < 0) {
                       pr_err("%s: gpio_direction_input fail rc=%d\n", __func__                                                                                        ,rc);
                       return rc ;
               }

               pinfo->te.te_irq = gpio_to_irq(pinfo->te.te_gpio);
               pr_info("%s:liyong  irq = %d\n", __func__, pinfo->te.te_irq);
               rc = request_threaded_irq(pinfo->te.te_irq, te_interrupt, NULL,
               ...
               pr_err("%s:liyong irq gpio not provided\n",__func__);
               return rc ;
       }
       return 0;
}
...
//BEGIN <LCM> <DATE20150203> <add TE check for lcm eds check> limi.zhang 
	 	if(ctrl_pdata->status_TE_mode==1) // WRW TE
	 	init_te_irq(pinfo); 
//END <LCM> <DATE20150203> <add TE check for lcm eds check> limi.zhang 
