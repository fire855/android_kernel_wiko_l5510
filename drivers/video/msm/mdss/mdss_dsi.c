...
int mdss_dsi_on(struct mdss_panel_data *pdata)
{
	...
	struct mipi_panel_info *mipi;
	struct mdss_dsi_ctrl_pdata *ctrl_pdata = NULL;
	#if defined(CONFIG_PROJECT_L9010)			//<Begin><FFBAKK-105><20141127>update l9010 LCD;xiongdajun
	u32 tmp;	
	#endif							//<END><FFBAKK-105><20141127>update l9010 LCD;xiongdajun
	if (pdata == NULL) {
		pr_err("%s: Invalid input data\n", __func__);
		return -EINVAL;
	}
	...
	//<Begin><FFBAKK-105><20141127>update l9010 LCD;xiongdajun
	...
	if (pdata->panel_info.type == MIPI_CMD_PANEL)
		mdss_dsi_clk_ctrl(ctrl_pdata, DSI_ALL_CLKS, 0);
	#endif
	//<END><FFBAKK-105><20141127>update l9010 LCD;xiongdajun
	pr_debug("%s-:\n", __func__);
	return 0;
}
				...
				struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	...
	if (ctrl_pdata->status_mode_two == ESD_REG)
		ctrl_pdata->check_status = mdss_dsi_reg_status_check;
