...
struct msm_tlmm_desc *gtlmm_desc = NULL;
static bool force_set_gp_clk = false;
#endif
...
#if defined(CONFIG_LEDS_GPIO_CLK)
	if(pin_no == 31 && force_set_gp_clk==false){
		pr_info("%s : skip set gpio_31 to  func\n", __func__);
		return ;
	}
#endif
...
#if defined(CONFIG_LEDS_GPIO_CLK)
int extern_set_gpio(int gpio_pin)
{	
	if(gtlmm_desc == NULL){
		return -1;
	}
	force_set_gp_clk = true;
	msm_tlmm_v4_gp_fn(gpio_pin, 2, gtlmm_desc->base, 1);
	force_set_gp_clk = false;	
	return 0;
}
#endif
