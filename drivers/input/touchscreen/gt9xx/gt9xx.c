...
//LINE <touchpanel> <DATE2015104> <remove large area sleep function> limi.zhang
...
//BEGIN <touchpanel> <DATE20141008> <goodix tp> limi.zhang
//extern char* g_tp_device_name;
//extern u8 g_tp_cfg_ver;
//END <touchpanel> <DATE20141008> <goodix tp> limi.zhang
...
//LINE <tp> <DATE20141104> <gt9xx> limi.zhang
u8 gtp_sensor_id = 0; 
...
//                send_cfg_buf[sensor_id][0] = 0x00;
	...
	/*[BUGFIX] ADD begin by TCTSZ-WH,2014-5-14,Show TP config version*/
    //LINE <touchpanel> <DATE20141008> <goodix tp> limi.zhang
...
//BEGIN <tp> <DATE20141027> <add tp info> limi.zhang
#if 1
extern int store_tp_info(const char *const str);
char gt9xx_tpd_fw_vendor_name[50]={0};
char gt9xx_tpd_ic_name[30]={0};
static u8 gt9xx_tpd_get_cfg_version( void )
{
    s32 ret = -1;
    //s32 i;
    struct goodix_ts_data *ts = NULL;
...
int gt9xx_tpd_get_fw_version( void )
{
    u16 fw_version = 0;
    struct goodix_ts_data *ts = NULL;
    ts = i2c_get_clientdata(i2c_connect_client);
    gtp_read_version(ts->client, &fw_version);	
    GTP_INFO("Gt9xx_tpd_get_fw_version: fw_version =  %x",fw_version);  
    return fw_version;
}

void gt9xx_tpd_get_fw_vendor_name(char * fw_vendor_name)
{
    u8 cfg_vs;
    cfg_vs = gt9xx_tpd_get_cfg_version();
    GTP_INFO("gt9xx_tpd_get_fw_vendor_name: cfg_vs =  %d",cfg_vs);
    switch(gtp_sensor_id)
    {
        case GT9XX_CTP_VENDOR_DIJING:
            sprintf(fw_vendor_name, "DIJING(%d)", cfg_vs);
	      break;
 
        case GT9XX_CTP_VENDOR_SHENYUE:
            //sprintf(fw_vendor_name, "SHENYUE(%d)", cfg_vs);
            sprintf(fw_vendor_name, "SHENYUE");
            break;
			
        case GT9XX_CTP_VENDOR_TCL:
            sprintf(fw_vendor_name, "TCL(%d)", cfg_vs);
            break;

        case GT9XX_CTP_VENDOR_JTOUCH:
            sprintf(fw_vendor_name, "JTOUCH(%d)", cfg_vs);
            break;
...
static int gt9xx_tpd_save_tp_info(void)
{
	char buf[80];
	int fw_version = gt9xx_tpd_get_fw_version();
    gt9xx_tpd_get_fw_vendor_name(gt9xx_tpd_fw_vendor_name);
    //SHENYUE-L5320-GT9157-1070
    sprintf(buf, "%s-%s-%s-%x",gt9xx_tpd_fw_vendor_name,CONFIG_PRODUCT_NAME, gt9xx_tpd_ic_name,fw_version);         
	store_tp_info(buf);
	return 0;
}

#endif
//END <tp> <DATE20141027> <add tp info> limi.zhang
			...
			struct goodix_ts_platform_data *pdata)
{
	...
	rc = of_property_read_string(np, "goodix,product-id",
						&pdata->product_id);
	//BEGIN <tp> <DATE20141104> <add tp info> limi.zhang
	//strcpy(gt9xx_tpd_ic_name, pdata->product_id);	
	sprintf(gt9xx_tpd_ic_name, "GT%s", pdata->product_id);							
	if (rc && (rc != -EINVAL)) {
...
gt9xx_tpd_save_tp_info();
printk("GTP goodix_ts_probe end");
