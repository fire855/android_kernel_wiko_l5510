...
//Add By YangRujin Start : store tp  info 
...
static int save_tp_info(char *product_id, char *config_id, int id, int rev_id)
{
	...
	//LINE<FFBAKK-502><20141104>Modify TP information;xiongdajun
        ...
        else if(!strncmp(product_id, "L5320_2C", sizeof("L5320_2C"))){
	...
	else if(!strncmp(product_id, "s3508", sizeof("DJTOUCH"))){
            sprintf(buf, "SAMSUNG-%s-S%d--V%d%d",
                    CONFIG_PRODUCT_NAME , id, config_id[2], config_id[3]);
        }
	#endif
        else{
...
#if 0
static unsigned char fw_config_id[4];//Add By YangRujin for store FW Ver.
#endif
...
//Modify By YangRujin Start: for reading FW ver
...
//Begin<REQ><20141224><TP update if in flash prog mode>;xiongdajun
...
//end<REQ><20141224><TP update if in flash prog mode>;xiongdajun
	if (img->is_contain_build_info) {
...
#if 1
	#if defined(CONFIG_PROJECT_L9010)	
	if ((imageFirmwareID != deviceFirmwareID) || (imageConfigID != deviceConfigID) ) {
		flash_area = UI_FIRMWARE;
		goto exit;
	} else {
		flash_area = NONE;
...
//Modify By YangRujin Start: for reading FW ver & IC ID
		...
		struct device_attribute *attr, char *buf)
{
	...
	return snprintf(buf, PAGE_SIZE, "%x.%x.%x.%x\n",
		config_id[0], config_id[1], config_id[2], config_id[3]);
#else		
	bool  static_fw_config_id_zero;
	static_fw_config_id_zero = ((fw_config_id[0]==fw_config_id[1])
			&&(fw_config_id[1]==fw_config_id[2])
			&&(fw_config_id[2]==fw_config_id[3])&&(fw_config_id[3]==0));

	if(static_fw_config_id_zero){
		unsigned char config_id[4];
		/* device config id */
		fwu->fn_ptr->read(fwu->rmi4_data,
					fwu->f34_fd.ctrl_base_addr,
...
{
#if 1
	if(fwu!=NULL && (fwu->rmi4_data)!=NULL){
		return snprintf(buf, PAGE_SIZE, "S%d rev %d\n",
			fwu->rmi4_data->rmi4_mod_info.package_id,
			fwu->rmi4_data->rmi4_mod_info.package_id_rev);
	}
