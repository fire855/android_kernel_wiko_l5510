...
/*
1. MODIFY add YangRujin 2014-9-18 : For fw image name setting.
*/
...
#define JTOUCH  "JTOUCH"
#define TTOUCH  "TTOUCH"
				...
				struct synaptics_rmi4_platform_data *rmi4_pdata)
{
	...
	rc = of_property_read_string(np, "synaptics,fw-image-name-jtouch",
		&rmi4_pdata->fw_image_name_jtouch);
	if (rc && (rc != -EINVAL)) {
		dev_err(dev, "Unable to read fw image name jtouch\n");
		return rc;
	}
	rc = of_property_read_string(np, "synaptics,fw-image-name-ttouch",
		&rmi4_pdata->fw_image_name_ttouch);
	if (rc && (rc != -EINVAL)) {
		dev_err(dev, "Unable to read fw image name for ttouch\n");
		return rc;
	}
	/* reset, irq gpio info */
					...
					struct synaptics_rmi4_data *rmi4_data)
{
	...
	dev_dbg(&rmi4_data->i2c_client->dev,
			"%s: manufacturer ID = %d, test id=%d, product id string=%s\n",
			__func__, rmi->manufacturer_id, rmi->tester_id, rmi->product_id_string);
	if(rmi4_data->board->fw_image_name == NULL){
		if(!strncmp(rmi->product_id_string, JTOUCH, sizeof(JTOUCH))){
			pr_info("%s : It is jtouch\n", __func__);
			rmi4_data->board->fw_image_name = rmi4_data->board->fw_image_name_jtouch;
		}else if(!strncmp(rmi->product_id_string, TTOUCH, sizeof(TTOUCH))){
			pr_info("%s : It is ttouch\n", __func__);
			rmi4_data->board->fw_image_name = rmi4_data->board->fw_image_name_ttouch;
		}else{
			pr_info("%s : Unkonwn touch\n", __func__);
			rmi4_data->board->fw_image_name = rmi4_data->board->fw_image_name_jtouch;
		}		
	}
	retval = synaptics_rmi4_i2c_read(rmi4_data,
	...
	dev_info(&rmi4_data->i2c_client->dev, "%s: package_id ID = %d\n",
			__func__, rmi->package_id);
	...
	#if 0
	for(i=0; i<=sizeof(rmi->build_id); i++){
		dev_err(&rmi4_data->i2c_client->dev, "%s: build_id[%d]  = %d\n",
				__func__, i, rmi->build_id[i]);		
	}
	#endif
		...
		const struct i2c_device_id *dev_id)
{
	...
	dev_dbg(&client->dev, "%s: fw image name = %s\n", __func__, rmi4_data->fw_image_name);
