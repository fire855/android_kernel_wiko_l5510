...
//extern void do_msm_poweroff(void);//yangliang add for charger and shutdown without battery
//<BEGIN><FFBAKK-105><2014.01.20>mask again for factory using without batt;yangliang
...
struct qpnp_lbc_chip {
	...
	//start add by lishengli 20141017	tinno charge temp manage
	bool				cfg_tinno_temp_enable;
	int 				cfg_tinno_high_temp;
	int				cfg_tinno_low_temp;
	int				cfg_tinno_recharge_hightemp;
	int				cfg_tinno_recharge_lowtemp;
	bool				tinno_recharge_flag;
	bool				tinno_discharge_flag;
	bool				tinno_temp_flag;
	//end add by lishengli 20141017	tinno charge temp manage
...
static int qpnp_lbc_bat_if_configure_btc(struct qpnp_lbc_chip *chip)
{
	...
	pr_info("BTC configuration mask=%x\n", btc_cfg);
...
static int qpnp_lbc_ibatmax_set(struct qpnp_lbc_chip *chip, int chg_current)
{
	...
	if (chg_current > QPNP_LBC_IBATMAX_MAX)
		pr_info("bad mA=%d clamping current\n", chg_current);
...
static int get_prop_current_now(struct qpnp_lbc_chip *chip)
{
	...
	if (chip->bms_psy) {
		...
		pr_info("No BMS supply registered return 0\n");
	}
...
static int get_prop_capacity(struct qpnp_lbc_chip *chip)
{
	...
	//yangliang add for charger and shutdown without battery
	//<BEGIN><FFBAKK-105><2014.01.20>mask again for factory using without batt;yangliang
	//if (!get_prop_batt_present(chip))
	//	do_msm_poweroff();

	//if (chip->cfg_use_fake_battery)//yangliang add for charger and shutdown without battery
	//<BEGIN><FFBAKK-105><2014.01.20>mask again for factory using without batt;yangliang
...
//start add by lishengli 20141017	tinno charge temp manage
#define BATTERY_COMMON_TEMPERATURE	0
#define BATTERY_OVER_TEMPERATURE 	1
#define BATTERY_UNDER_TEMPERATURE	5
#define BATTERY_RESUME_TEMPERATURE	9
static unsigned int g_TinnoBatteryNotifyCode = BATTERY_COMMON_TEMPERATURE;
...
{
    pr_info("[BatteryNotify] show_BatteryNotify : %d\n", g_TinnoBatteryNotifyCode);
...
static ssize_t store_BatteryNotify(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
    ...
    pr_info("[BatteryNotify] store_BatteryNotify\n");
    if(buf != NULL && size != 0)
    {
        pr_info("[BatteryNotify] buf is %s and size is %d \n",buf,size);
        reg_BatteryNotifyCode = simple_strtoul(buf,&pvalue,16);
        g_TinnoBatteryNotifyCode = reg_BatteryNotifyCode;
        pr_info("[BatteryNotify] store code : %d \n",g_TinnoBatteryNotifyCode);        
    }        
    return size;
}
...
//start add by lishengli 20141017	tinno charge temp manage
	chip->tinno_recharge_flag = false;
	chip->tinno_discharge_flag = false;

	if(chip->cfg_tinno_temp_enable)
	{
		if (state == ADC_TM_WARM_STATE) {
			if(temp >= chip->cfg_tinno_high_temp)
			{
				/* warm to tinno_high_temp */
				bat_warm = true;
				bat_cool = false;
				chip->tinno_discharge_flag = true;
				chip->tinno_temp_flag = true;
				
				chip->adc_param.low_temp =
						chip->cfg_tinno_recharge_hightemp;
			...
			else if(temp >= chip->cfg_tinno_recharge_lowtemp)
			{
				/* tinno_low_temp to tinno_low_recharge */
				bat_warm = false;
				bat_cool = true;

				if(chip->tinno_temp_flag)
				{
					chip->tinno_recharge_flag = true;
					chip->tinno_temp_flag = false;
				}

				chip->adc_param.low_temp =
						chip->cfg_tinno_low_temp;
				chip->adc_param.high_temp =
						chip->cfg_cool_bat_decidegc + HYSTERISIS_DECIDEGC;
				chip->adc_param.state_request =
						ADC_TM_HIGH_LOW_THR_ENABLE;
			...
			if(temp <= chip->cfg_tinno_low_temp)
			{
				/* cool to tinno_low_temp */
				bat_warm = false;
				bat_cool = true;
				chip->tinno_discharge_flag = true;
				chip->tinno_temp_flag = true;
				...
				chip->adc_param.high_temp =
						chip->cfg_tinno_recharge_lowtemp;
				chip->adc_param.state_request =
			...
			else if(temp <= chip->cfg_tinno_recharge_hightemp)
			{
				/* tinno_high_temp to tinno_recharge_temp */
				bat_warm = true;			
				bat_cool = false;
...
// end add by lishengli 20141017	tinno charge temp manage
	...
	pr_info("warm %d, cool %d, bat_warm %d, bat_cool %d, low = %d deciDegC, high = %d deciDegC\n",
...
// start add by lishengli 20141017	tinno charge temp manage
	pr_info("cfg_tinno_temp_enable %d, tinno_discharge_flag %d, tinno_recharge_flag = %d \n",
			chip->cfg_tinno_temp_enable, chip->tinno_discharge_flag,
			chip->tinno_recharge_flag);

	if(chip->cfg_tinno_temp_enable)
	{
		spin_lock_irqsave(&chip->ibat_change_lock, flags);	
		
		if(chip->tinno_discharge_flag)
		{
			g_TinnoBatteryNotifyCode = temp >= chip->cfg_tinno_high_temp ? BATTERY_OVER_TEMPERATURE : BATTERY_UNDER_TEMPERATURE;
			qpnp_lbc_charger_enable(chip, THERMAL, 0);
			power_supply_changed(&chip->batt_psy);
		}
		else if(chip->tinno_recharge_flag)
		{
			g_TinnoBatteryNotifyCode = BATTERY_RESUME_TEMPERATURE;
			qpnp_lbc_set_appropriate_vddmax(chip);
			qpnp_lbc_set_appropriate_current(chip);
			...
			g_TinnoBatteryNotifyCode = BATTERY_COMMON_TEMPERATURE;

		pr_info("g_TinnoBatteryNotifyCode %d \n", g_TinnoBatteryNotifyCode);
...
static int qpnp_lbc_misc_init(struct qpnp_lbc_chip *chip)
{
	...
	if (reg_val >= LBC_MISC_DIG_VERSION_1) {
		...
		pr_info("Initial trim center %x trim_uv %d\n",
...
static int qpnp_charger_read_dt_props(struct qpnp_lbc_chip *chip)
{
	...
	//start add by lishengli 20141017	tinno charge temp manage
	OF_PROP_READ(chip, cfg_tinno_high_temp, "tinno-high-temp", rc, 1);
	OF_PROP_READ(chip, cfg_tinno_low_temp, "tinno-low-temp", rc, 1);
	OF_PROP_READ(chip, cfg_tinno_recharge_hightemp, "tinno-recharge-hightemp", rc, 1);
	OF_PROP_READ(chip, cfg_tinno_recharge_lowtemp, "tinno-recharge-lowtemp", rc, 1);

	/* Get the tinno temp manage enable property */
	chip->cfg_tinno_temp_enable= of_property_read_bool(
			chip->spmi->dev.of_node, "qcom,tinno-temp-enable");
	//end add by lishengli 20141017	tinno charge temp manage
...
//<BEGIN><><20140918>wangyanhui
/*#define CHG_MAX_RED_BRIGHTNESS  255
static int led_red_registered = 0;
static void qpnp_chg_led_set(struct led_classdev *led_cdev,
				      enum led_brightness value)
{
	struct qpnp_lbc_chip *chip = container_of(led_cdev, struct qpnp_lbc_chip, cdev);
	int rc;
...
static int qpnp_lbc_probe(struct spmi_device *spmi)
{
	...
	chip->tinno_temp_flag = false;		// add by lishengli 20141017	tinno charge temp manage
	...
	//<BEGIN><><20140918>wangyanhui
	/*if(!led_red_registered)
	{
		chip->cdev.name = "red";
		chip->cdev.brightness = 0;
		chip->cdev.max_brightness = CHG_MAX_RED_BRIGHTNESS;
		chip->cdev.default_trigger = "battery-charging";
...
//start add by lishengli 20141017	tinno charge temp manage	
static int tn_batteryNotify_probe(struct platform_device *dev)    
{
    int ret_device_file = 0;
    //struct proc_dir_entry *entry = NULL;
    //struct proc_dir_entry *battery_dir = NULL;

    pr_info("******** tn_batteryNotify_probe!! ********\n" );


    ret_device_file = device_create_file(&(dev->dev), &dev_attr_BatteryNotify);

#if 0    
    battery_dir = proc_mkdir("tn_battery_cmd", NULL);
    if (!battery_dir)
    {
        pr_info("[%s]: mkdir /proc/tn_battery_cmd failed\n", __FUNCTION__);
    }
    else
    {
        #if 1
        proc_create("battery_cmd", S_IRUGO | S_IWUSR, battery_dir, &battery_cmd_proc_fops);
        pr_info("proc_create battery_cmd_proc_fops\n");
        #else
...
struct platform_device TN_batteryNotify_device = {
    .name   = "tinno-battery",
    .id        = -1,
};

static struct platform_driver TN_batteryNotify_driver = {
    .probe        = tn_batteryNotify_probe,
    .driver     = {
        .name = "tinno-battery",
    },
};
...
//start add by lishengli 20141017	tinno charge temp manage	
	...
	// battery notofy UI
	ret = platform_device_register(&TN_batteryNotify_device);
	if (ret) {
		pr_info("****[batteryNotify] Unable to device register(%d)\n", ret);
		return ret;
	}    
	
	ret = platform_driver_register(&TN_batteryNotify_driver);
	if (ret) {
		pr_info( "****[batteryNotify] Unable to register driver (%d)\n", ret);
		return ret;
	}
//end add by lishengli 20141017	tinno charge temp manage	
