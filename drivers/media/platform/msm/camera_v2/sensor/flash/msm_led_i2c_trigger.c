...
{
	int rc = 0;
	static uint16_t localData = 0xAB;   //LINE <> <20150113> <avoid the flash IC lowvoltage problem> panzaoyan
	...
	if (fctrl->flash_i2c_client && fctrl->reg_setting) {
		...
		if (rc < 0)
 	     ...
 	     //BEGIN <> <20150113> <avoid the flash IC lowvoltage problem> panzaoyan
     	fctrl->flash_i2c_client->i2c_func_tbl->i2c_read( 
        fctrl->flash_i2c_client, 
        0x0B,&localData,MSM_CAMERA_I2C_BYTE_DATA); 
	    //pr_err("panzaoyan the localdata is 0x%x\n",localData);
	     //END <> <20150113> <avoid the flash IC lowvoltage problem> panzaoyan
